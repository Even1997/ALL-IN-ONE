...
//! to communicate with GoodNight's HTTP-based MCP server.
//!
...
/// Resolve the platform-specific GoodNight data directory.
/// Must match Tauri's `app.path().app_data_dir()` for identifier `com.goodnight.app`.
fn goodnight_data_dir() -> Option<PathBuf> {
    dirs::data_dir().map(|d| d.join("com.goodnight.app"))
}

/// Discover the auth token: GOODNIGHT_TOKEN env var, then local token file on disk.
fn discover_token() -> Option<String> {
    if let Ok(token) = env::var("GOODNIGHT_TOKEN") {
        if !token.is_empty() {
            return Some(token);
        }
    }

    let data_dir = goodnight_data_dir()?;
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "goodnight_mcp_bridge=info,warn".parse().unwrap()),
        )
