...
/// Shared test context 鈥?holds the temp dir (so the DB stays alive),
/// the app state, and the raw auth token.
struct TestCtx {
    _temp: tempfile::TempDir,
    state: web::Data<goodnight_server::state::AppState>,
    token: String,
}
...
impl TestCtx {
    async fn new() -> Self {
        ...
        let manager = Arc::new(goodnight_core::DatabaseManager::new(temp.path()).unwrap());
        ...
        let state = web::Data::new(goodnight_server::state::AppState {
            manager,
            event_tx,
            public_url: None,
            log_buffer: goodnight_server::log_buffer::LogBuffer::new(16),
            export_jobs: goodnight_server::export_jobs::ExportJobManager::for_tests(
...
> {
    App::new()
        ...
        .route(
            ...
            web::get().to(goodnight_server::routes::exports::download_export),
        )
        .service(
            web::scope("/api")
                .wrap(goodnight_server::auth::BearerAuth {
                    state: ctx.state.clone(),
                })
                .configure(goodnight_server::routes::configure_routes),
        )
}
