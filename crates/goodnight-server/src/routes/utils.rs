...
pub async fn compact_tags(db: Db) -> HttpResponse {
    ...
    let (provider_config, model) = {
        ...
        let provider_config = goodnight_core::ProviderConfig::from_settings(&settings_map);
        let model = match provider_config.provider_type {
            goodnight_core::ProviderType::Ollama => provider_config.llm_model().to_string(),
            goodnight_core::ProviderType::OpenAICompat => provider_config.llm_model().to_string(),
            goodnight_core::ProviderType::OpenRouter => settings_map
                .get("tagging_model")
                .cloned()
    ...
    let supported_params: Option<Vec<String>> =
        if provider_config.provider_type == goodnight_core::ProviderType::OpenRouter {
            use goodnight_core::providers::models::fetch_and_return_capabilities;
    ...
    match goodnight_core::compaction::fetch_merge_suggestions(
        &provider_config,
        &all_tags,
        &model,
        supported_params,
