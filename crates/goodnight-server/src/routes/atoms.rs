...
pub async fn get_atoms(db: Db, query: web::Query<GetAtomsQuery>) -> HttpResponse {
    let source_filter = match query.source.as_deref() {
        Some("manual") => goodnight_core::SourceFilter::Manual,
        Some("external") => goodnight_core::SourceFilter::External,
        _ => goodnight_core::SourceFilter::All,
    };
    let sort_by = match query.sort_by.as_deref() {
        Some("created") => goodnight_core::SortField::Created,
        Some("published") => goodnight_core::SortField::Published,
        Some("title") => goodnight_core::SortField::Title,
        _ => goodnight_core::SortField::Updated,
    };
    let sort_order = match query.sort_order.as_deref() {
        Some("asc") => goodnight_core::SortOrder::Asc,
        _ => goodnight_core::SortOrder::Desc,
    };
    let params = goodnight_core::ListAtomsParams {
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Recent atoms or title matches for editor link completion", body = Vec<goodnight_core::AtomLinkSuggestion>),
        (status = 500, description = "Internal error", body = ApiErrorResponse),
    ),
    tag = "atoms",
)]
...
pub async fn create_atom(
    ...
    match db
        .0
        .create_atom(
            goodnight_core::CreateAtomRequest {
                content: req.content,
...
pub async fn bulk_create_atoms(
    ...
    let requests: Vec<goodnight_core::CreateAtomRequest> = body
        .into_inner()
        .into_iter()
        .map(|r| goodnight_core::CreateAtomRequest {
            content: r.content,
...
pub async fn update_atom(
    ...
    match db
        ...
        .update_atom(
            &id,
            goodnight_core::UpdateAtomRequest {
                content: req.content,
                source_url: req.source_url,
                published_at: req.published_at,
                tag_ids: req.tag_ids,
