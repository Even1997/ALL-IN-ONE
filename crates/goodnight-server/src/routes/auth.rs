...
#[utoipa::path(post, path = "/api/auth/tokens", request_body = CreateTokenBody, responses((status = 201, description = "Token created (includes raw token 鈥?save it, won't be shown again)")), tag = "auth")]
...
#[utoipa::path(get, path = "/api/auth/tokens", responses((status = 200, description = "List of API tokens (metadata only)", body = Vec<goodnight_core::ApiTokenInfo>)), tag = "auth")]
