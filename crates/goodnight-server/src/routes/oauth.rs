    ...
    let public_url = match &state.public_url {
        ...
        None => {
            return HttpResponse::NotFound().json(serde_json::json!({
                "error": "OAuth not configured 鈥?start server with --public-url"
            }))
        }
    };
...
<script>
  ...
  let stored = null;
  try {{
    const config = JSON.parse(localStorage.getItem('goodnight-server-config') || 'null');
...
mod tests {
    ...
    fn test_state_with_oauth() -> web::Data<AppState> {
        ...
        let manager = std::sync::Arc::new(goodnight_core::DatabaseManager::new(temp.path()).unwrap());
    ...
    async fn test_code_cannot_be_reused() {
        ...
        // First exchange 鈥?should succeed
        let req = actix_test::TestRequest::post()
            .uri("/oauth/token")
            .insert_header(("Content-Type", "application/x-www-form-urlencoded"))
        ...
        // Second exchange with same code 鈥?should fail
        let req = actix_test::TestRequest::post()
            .uri("/oauth/token")
            .insert_header(("Content-Type", "application/x-www-form-urlencoded"))
