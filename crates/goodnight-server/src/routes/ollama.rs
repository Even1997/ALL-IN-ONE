...
pub async fn test_ollama(body: web::Json<TestOllamaBody>) -> HttpResponse {
    match goodnight_core::providers::models::test_ollama_connection(&body.host).await {
...
pub async fn get_ollama_models(query: web::Query<OllamaHostQuery>) -> HttpResponse {
    ...
    match goodnight_core::providers::models::fetch_ollama_models(host).await {
...
pub async fn get_ollama_embedding_models(query: web::Query<OllamaHostQuery>) -> HttpResponse {
    let host = query.host.as_deref().unwrap_or("http://127.0.0.1:11434");
    match goodnight_core::providers::models::get_ollama_embedding_models(host).await {
        Ok(models) => HttpResponse::Ok().json(models),
        Err(e) => HttpResponse::BadGateway().json(serde_json::json!({"error": e})),
    }
}
...
pub async fn get_ollama_llm_models(query: web::Query<OllamaHostQuery>) -> HttpResponse {
    let host = query.host.as_deref().unwrap_or("http://127.0.0.1:11434");
    match goodnight_core::providers::models::get_ollama_llm_models(host).await {
        Ok(models) => HttpResponse::Ok().json(models),
        Err(e) => HttpResponse::BadGateway().json(serde_json::json!({"error": e})),
    }
}
...
pub async fn verify_provider_configured(db: Db) -> HttpResponse {
    ...
    let config = goodnight_core::ProviderConfig::from_settings(&settings);

    let configured = match config.provider_type {
        goodnight_core::ProviderType::OpenRouter => config
            .openrouter_api_key
            .as_ref()
            .map(|k| !k.is_empty())
            .unwrap_or(false),
        goodnight_core::ProviderType::Ollama => !config.ollama_host.is_empty(),
        goodnight_core::ProviderType::OpenAICompat => !config.openai_compat_base_url.is_empty(),
    };
