...
#[utoipa::path(get, path = "/api/feeds", responses((status = 200, description = "All feeds", body = Vec<goodnight_core::Feed>)), tag = "feeds")]
pub async fn list_feeds(db: Db) -> HttpResponse {
    ok_or_error(db.0.list_feeds().await)
}

#[utoipa::path(get, path = "/api/feeds/{id}", params(("id" = String, Path, description = "Feed ID")), responses((status = 200, description = "Feed details", body = goodnight_core::Feed), (status = 404, description = "Feed not found", body = ApiErrorResponse)), tag = "feeds")]
pub async fn get_feed(db: Db, path: web::Path<String>) -> HttpResponse {
    let id = path.into_inner();
    ok_or_error(db.0.get_feed(&id).await)
}

#[utoipa::path(post, path = "/api/feeds", request_body = goodnight_core::CreateFeedRequest, responses((status = 201, description = "Feed created", body = goodnight_core::Feed)), tag = "feeds")]
pub async fn create_feed(
    state: web::Data<AppState>,
    db: Db,
    body: web::Json<goodnight_core::CreateFeedRequest>,
) -> HttpResponse {
...
#[utoipa::path(put, path = "/api/feeds/{id}", params(("id" = String, Path, description = "Feed ID")), request_body = goodnight_core::UpdateFeedRequest, responses((status = 200, description = "Feed updated", body = goodnight_core::Feed)), tag = "feeds")]
pub async fn update_feed(
    db: Db,
    path: web::Path<String>,
    body: web::Json<goodnight_core::UpdateFeedRequest>,
) -> HttpResponse {
    let id = path.into_inner();
