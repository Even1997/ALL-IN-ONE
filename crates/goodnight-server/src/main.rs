//! Atomic Server 鈥?standalone HTTP server for the Atomic knowledge base
//!
//! Wraps goodnight-core with a REST API + WebSocket events.
//! No Tauri dependency.
...
async fn main() -> std::io::Result<()> {
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| "goodnight_core=info,goodnight_server=info,warn".parse().unwrap());
...
) -> goodnight_core::DatabaseManager {
    match storage {
        "postgres" => {
            ...
            goodnight_core::DatabaseManager::new_postgres(data_dir, url)
                .await
                .expect("Failed to connect to Postgres")
        }
        _ => {
            tracing::info!(backend = "sqlite", path = %data_dir.display(), "storage backend selected");
            goodnight_core::DatabaseManager::new(data_dir).expect("Failed to open database manager")
        }
    }
}

async fn run_token_command(core: &goodnight_core::AtomicCore, action: TokenAction) {
    match action {
        TokenAction::Create { name } => match core.create_api_token(&name).await {
            Ok((info, raw_token)) => {
                ...
                println!();
                println!("Save this token 鈥?it won't be shown again.");
            }
            Err(e) => {
...
async fn run_server(
    manager: goodnight_core::DatabaseManager,
    data_dir: &str,
    port: u16,
    bind: &str,
    ...
    match core.list_api_tokens().await {
        Ok(tokens) => {
            ...
            if active == 0 {
                tracing::info!("no API tokens configured 鈥?open the web UI to claim this instance or run: goodnight-server token create --name default");
            } else {
    ...
    // Spawn feed polling scheduler (ticks every 60 seconds, polls all databases)
    {
        ...
        tokio::spawn(async move {
            ...
            interval.tick().await; // first tick fires immediately 鈥?skip it
            loop {
                interval.tick().await;
    ...
    // next tick from re-entering a still-running task.
    {
        ...
        tokio::spawn(async move {
            let mut registry = goodnight_core::scheduler::TaskRegistry::new();
            registry.register(Arc::new(goodnight_core::briefing::DailyBriefingTask));
            registry.register(Arc::new(goodnight_core::pipeline_task::DraftPipelineTask));
            registry.register(Arc::new(
                goodnight_core::graph_maintenance::GraphMaintenanceTask,
            ));
            ...
            loop {
                ...
                for db_info in &databases {
                    ...
                    for task in registry.tasks() {
                        ...
                        tokio::spawn(async move {
                            let ctx = goodnight_core::scheduler::TaskContext {
    ...
    HttpServer::new(move || {
        ...
        App::new()
            ...
            .route(
                ...
                web::get().to(goodnight_server::openapi_spec),
            )
            .service(Scalar::with_url(
                "/api/docs",
                goodnight_server::ApiDoc::openapi(),
            ))
            ...
            // Instance setup (public, no auth 鈥?guarded by zero-token check)
            .route(
                "/api/setup/status",
    ...
    tracing::info!("shutting down 鈥?running PRAGMA optimize...");
