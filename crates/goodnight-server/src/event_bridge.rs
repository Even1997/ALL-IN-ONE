//! Bridge between goodnight-core callback events and the server broadcast channel
//!
...
pub fn embedding_event_callback(
    ...
    move |event: goodnight_core::EmbeddingEvent| {
...
pub fn ingestion_event_callback(
    tx: broadcast::Sender<ServerEvent>,
) -> impl Fn(goodnight_core::IngestionEvent) + Send + Sync + Clone + 'static {
    move |event: goodnight_core::IngestionEvent| {
        let _ = tx.send(ServerEvent::from(event));
    }
}
...
pub fn chat_event_callback(
    tx: broadcast::Sender<ServerEvent>,
) -> impl Fn(goodnight_core::ChatEvent) + Send + Sync + 'static {
    move |event: goodnight_core::ChatEvent| {
        let _ = tx.send(ServerEvent::from(event));
    }
}
...
pub fn task_event_callback(
    tx: broadcast::Sender<ServerEvent>,
) -> Arc<dyn Fn(goodnight_core::scheduler::TaskEvent) + Send + Sync> {
    Arc::new(move |event: goodnight_core::scheduler::TaskEvent| {
        use goodnight_core::scheduler::TaskEvent;
        match event {
            TaskEvent::Completed {
                task_id,
                db_id,
