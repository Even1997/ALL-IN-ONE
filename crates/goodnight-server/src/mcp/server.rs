...
use goodnight_core::manager::DatabaseManager;
...
impl AtomicMcpServer {
    ...
    async fn semantic_search(
        ...
        let options =
            goodnight_core::SearchOptions::new(params.query, goodnight_core::SearchMode::Hybrid, limit)
                .with_threshold(0.3)
    ...
    #[tool(
        description = "Remember something new. Create an atom when you learn information worth retaining across conversations 鈥?user preferences, decisions, project context, or important facts. Write concise, self-contained markdown."
    )]
    async fn create_atom(
        ...
        let core = self.resolve_core(&context).await?;
        let request = goodnight_core::CreateAtomRequest {
    ...
    async fn update_atom(
        ...
        let request = goodnight_core::UpdateAtomRequest {
