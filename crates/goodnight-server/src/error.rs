//! API error handling 鈥?maps AtomicCoreError to HTTP responses
...
pub fn error_response(e: goodnight_core::AtomicCoreError) -> HttpResponse {
    match &e {
        goodnight_core::AtomicCoreError::NotFound(_) => {
            HttpResponse::NotFound().json(ApiErrorResponse {
                error: e.to_string(),
            })
        }
        goodnight_core::AtomicCoreError::Validation(_) => {
            HttpResponse::BadRequest().json(ApiErrorResponse {
                error: e.to_string(),
            })
        }
        goodnight_core::AtomicCoreError::Configuration(_) => {
            HttpResponse::BadRequest().json(ApiErrorResponse {
                error: e.to_string(),
            })
        }
        goodnight_core::AtomicCoreError::Conflict(_) => {
...
pub fn ok_or_error<T: serde::Serialize>(
    result: Result<T, goodnight_core::AtomicCoreError>,
) -> HttpResponse {
    match result {
...
pub fn status_code_for(e: &goodnight_core::AtomicCoreError) -> actix_web::http::StatusCode {
    use actix_web::http::StatusCode;
    match e {
        goodnight_core::AtomicCoreError::NotFound(_) => StatusCode::NOT_FOUND,
        goodnight_core::AtomicCoreError::Validation(_) => StatusCode::BAD_REQUEST,
        goodnight_core::AtomicCoreError::Configuration(_) => StatusCode::BAD_REQUEST,
        goodnight_core::AtomicCoreError::Conflict(_) => StatusCode::CONFLICT,
...
mod tests {
    use super::*;
    use actix_web::http::StatusCode;
    use goodnight_core::AtomicCoreError;
