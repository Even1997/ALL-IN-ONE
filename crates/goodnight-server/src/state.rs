...
use goodnight_core::{AtomicCore, DatabaseManager};
use serde::Serialize;
use std::sync::Arc;
...
impl AppState {
    ...
    /// Checks X-Goodnight-Database header, then ?db= query param, then falls back to active.
    pub async fn resolve_core(
        &self,
        req: &actix_web::HttpRequest,
    ) -> Result<AtomicCore, goodnight_core::AtomicCoreError> {
        // Check X-Goodnight-Database header
        if let Some(db_id) = req
            .headers()
            .get("X-Goodnight-Database")
...
pub enum ServerEvent {
    ...
    AtomCreated {
        atom: goodnight_core::AtomWithTags,
    },
    AtomUpdated {
        atom: goodnight_core::AtomWithTags,
    },
    ...
    ChatComplete {
        conversation_id: String,
        message: goodnight_core::ChatMessageWithContext,
    },
    ChatCanvasAction {
        conversation_id: String,
        action: String,
...
impl From<goodnight_core::EmbeddingEvent> for ServerEvent {
    fn from(event: goodnight_core::EmbeddingEvent) -> Self {
        match event {
            goodnight_core::EmbeddingEvent::Started { atom_id } => {
                ServerEvent::EmbeddingStarted { atom_id }
            }
            goodnight_core::EmbeddingEvent::EmbeddingComplete { atom_id } => {
                ServerEvent::EmbeddingComplete { atom_id }
            }
            goodnight_core::EmbeddingEvent::EmbeddingFailed { atom_id, error } => {
                ServerEvent::EmbeddingFailed { atom_id, error }
            }
            goodnight_core::EmbeddingEvent::TaggingComplete {
                atom_id,
                tags_extracted,
                new_tags_created,
            ...
            goodnight_core::EmbeddingEvent::TaggingFailed { atom_id, ref error } => {
            ...
            goodnight_core::EmbeddingEvent::TaggingSkipped { atom_id } => {
                ServerEvent::TaggingSkipped { atom_id }
            }
            goodnight_core::EmbeddingEvent::BatchProgress {
                batch_id,
                phase,
                completed,
                total,
            ...
            goodnight_core::EmbeddingEvent::PipelineQueueStarted {
                run_id,
                total_jobs,
                embedding_total,
            ...
            goodnight_core::EmbeddingEvent::PipelineQueueProgress {
                run_id,
                stage,
                completed,
                total,
            ...
            goodnight_core::EmbeddingEvent::PipelineQueueCompleted {
                run_id,
                total_jobs,
                failed_jobs,
...
impl From<goodnight_core::IngestionEvent> for ServerEvent {
    fn from(event: goodnight_core::IngestionEvent) -> Self {
        match event {
            goodnight_core::IngestionEvent::FetchStarted { url, request_id } => {
                ServerEvent::IngestionFetchStarted { url, request_id }
            }
            goodnight_core::IngestionEvent::FetchComplete {
                url,
                request_id,
                content_length,
            ...
            goodnight_core::IngestionEvent::FetchFailed {
                url,
                request_id,
                error,
            ...
            goodnight_core::IngestionEvent::Skipped {
                url,
                request_id,
                reason,
            ...
            goodnight_core::IngestionEvent::IngestionComplete {
                request_id,
                atom_id,
                url,
                title,
            ...
            goodnight_core::IngestionEvent::IngestionFailed {
                request_id,
                url,
                error,
            ...
            goodnight_core::IngestionEvent::FeedPollComplete {
                feed_id,
                new_items,
                skipped,
                errors,
            ...
            goodnight_core::IngestionEvent::FeedPollFailed { feed_id, error } => {
...
impl From<goodnight_core::ChatEvent> for ServerEvent {
    fn from(event: goodnight_core::ChatEvent) -> Self {
        match event {
            goodnight_core::ChatEvent::StreamDelta {
                conversation_id,
                content,
            ...
            goodnight_core::ChatEvent::ToolStart {
                conversation_id,
                tool_call_id,
                tool_name,
                tool_input,
            ...
            goodnight_core::ChatEvent::ToolComplete {
                conversation_id,
                tool_call_id,
                results_count,
            ...
            goodnight_core::ChatEvent::Complete {
                conversation_id,
                message,
            ...
            goodnight_core::ChatEvent::CanvasAction {
                conversation_id,
                action,
                params,
            ...
            goodnight_core::ChatEvent::AtomCreated {
                conversation_id: _,
                atom,
            } => ServerEvent::AtomCreated { atom },
            goodnight_core::ChatEvent::AtomUpdated {
                conversation_id: _,
                atom,
            } => ServerEvent::AtomUpdated { atom },
            goodnight_core::ChatEvent::AtomPipelineEvent {
                conversation_id: _,
                event,
            } => ServerEvent::from(event),
            goodnight_core::ChatEvent::Error {
                conversation_id,
                error,
...
mod tests {
    ...
    fn test_embedding_started_conversion() {
        let event = goodnight_core::EmbeddingEvent::Started {
            atom_id: "a1".into(),
        };
    ...
    fn test_embedding_complete_conversion() {
        let event = goodnight_core::EmbeddingEvent::EmbeddingComplete {
            atom_id: "a2".into(),
        };
    ...
    fn test_embedding_failed_conversion() {
        let event = goodnight_core::EmbeddingEvent::EmbeddingFailed {
            atom_id: "a3".into(),
            error: "timeout".into(),
        };
        match ServerEvent::from(event) {
    ...
    fn test_tagging_complete_conversion() {
        let event = goodnight_core::EmbeddingEvent::TaggingComplete {
            atom_id: "a4".into(),
    ...
    fn test_chat_stream_delta_conversion() {
        let event = goodnight_core::ChatEvent::StreamDelta {
    ...
    fn test_chat_tool_start_conversion() {
        let event = goodnight_core::ChatEvent::ToolStart {
    ...
    fn test_chat_error_conversion() {
        let event = goodnight_core::ChatEvent::Error {
