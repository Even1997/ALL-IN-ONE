...
/// form `{"tags": [...]}` and a bare array `[...]` 鈥?some local/OSS providers
...
pub fn get_tag_tree_cached(conn: &Connection, db_path: &str) -> Result<String, String> {
    ...
    // Check cache 鈥?must match both TTL and database path
    ...
    // Cache miss or stale 鈥?fetch fresh
...
2. Prefer 2-level hierarchy: [Category] 鈫?[Specific Tag]
3. Merge similar/overlapping tags (e.g., "AI Consciousness" + "AI Systems" 鈫?"AI")
...
/// Default maximum characters to send for tagging (~30K tokens 鈮?120K chars)
...
/// This replaces the per-chunk approach 鈥?the LLM sees the complete content
