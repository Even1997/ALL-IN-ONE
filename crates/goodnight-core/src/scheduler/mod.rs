//! Scheduled-task framework for goodnight-core.
//!
//! This module defines a minimal scheduling primitive that any transport
//! (goodnight-server, Tauri sidecar, etc.) can drive from its own runtime. The
...
//! persistence, and event reporting 鈥?see [`ScheduledTask::run`].
...
/// own event channel (see `goodnight-server::event_bridge::task_event_callback`).
#[derive(Debug, Clone)]
pub enum TaskEvent {
    Started {
        task_id: String,
