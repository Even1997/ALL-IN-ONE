...
//! algebra crate needed 鈥?pure Rust with O(d) working memory.
...
pub fn compute_2d_projection(embeddings: &[(String, Vec<f32>)]) -> Vec<(String, f64, f64)> {
    ...
    // the user switches embedding models 鈥?we project only the majority set and
    ...
    // Computes X^T(Xv) by streaming rows 鈥?O(d) working memory
...
fn power_iteration(
    ...
    // Initialize with a deterministic vector (not random 鈥?reproducible results)
    ...
    for _ in 0..max_iter {
        ...
        // First pass: compute Xv (n dot products 鈫?n scalars)
