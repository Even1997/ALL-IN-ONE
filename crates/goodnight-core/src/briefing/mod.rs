...
//! which mirrors the wiki agentic strategy 鈥?an LLM with a tiny tool set
...
/// A briefing joined with its citations 鈥?the primary read shape.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[cfg_attr(feature = "openapi", derive(utoipa::ToSchema))]
...
/// 2. If zero atoms: return `Ok(None)` 鈥?nothing is persisted. The caller
...
pub async fn run_briefing(
    ...
    if new_atoms.is_empty() {
        tracing::info!("[briefing] No new atoms 鈥?skipping briefing creation");
        return Ok(None);
    }
