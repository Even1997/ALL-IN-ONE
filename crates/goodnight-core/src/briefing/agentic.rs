...
//! against the *initial* new-atoms list 鈥?references the agent pulled in
...
/// daily briefing doesn't need deep research 鈥?most runs should terminate
...
/// `read_atom` tool (crates/goodnight-server/src/mcp/server.rs) so the agent sees
...
- **read_atom(atom_id, limit?, offset?)**: Read a window of lines from an atom's markdown content. Use this when the title and snippet aren't enough to summarize. Returns up to `limit` lines starting at `offset` (default: first 500 lines). For long atoms, page through with offset 鈥?do not try to read everything.
- **semantic_search(query, limit)**: Search the full knowledge base (new + existing atoms) to find related material. Use this sparingly 鈥?typically only when a new atom references something that sounds like it should connect to older notes.
...
fn briefing_tools() -> Vec<ToolDefinition> {
    vec![
        ...
        ToolDefinition::new(
            "semantic_search",
            "Search the full knowledge base for related material. Returns atom titles and snippets for context only 鈥?you cannot cite these atoms.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "query": {
...
async fn resolve_model(core: &AtomicCore) -> Result<(ProviderConfig, String), String> {
    ...
    // OpenRouter uses the wiki_model setting (the briefing shares it 鈥?intentional).
...
/// the prompt-based fallback lives there 鈥?this function is now just glue.
...
/// with a warning 鈥?this is the spec'd behavior for "agent cites something
/// it doesn't have."
...
pub(crate) async fn generate(
    ...
    // `briefing_content` field 鈥?the model needs to know this, otherwise it
...
mod tests {
    ...
    fn truncate_on_char_boundary_multibyte() {
        // "h茅llo" is 6 bytes (茅 is 2 bytes). Truncating at byte offset 3
        // must not split the 茅 codepoint 鈥?we should land at a safe boundary.
        let out = truncate_on_char_boundary("h茅llo", 3);
        assert!(out.ends_with("..."));
        // The non-ellipsis prefix must be valid UTF-8 (implicit 鈥?it's a String),
        // and it should contain either "h" or "h茅" but never a broken 茅.
    ...
    fn extract_citations_drops_out_of_range() {
        // Agent hallucinated [5] when only 2 atoms exist 鈥?must be dropped
        // without panicking.
        let atoms = vec![
            mock_atom("a-1", "first", "body 1"),
            mock_atom("a-2", "second", "body 2"),
