...
/// This is the precise, slower path 鈥?used for LLM context budgets, NOT for chunking.
        ...
        match event {
            ...
            Event::End(TagEnd::CodeBlock) => {
                match current_block {
                    Some((BlockType::List, _, ref mut end)) => {
                        // Code block ended inside list 鈥?just extend the list range
                        *end = range.end;
                    }
            ...
            Event::End(TagEnd::Paragraph) => {
                match current_block {
                    ...
                    _ => {
                        // Paragraph inside list/blockquote 鈥?extend parent
                        if let Some((_, _, ref mut end)) = current_block {
                            *end = range.end;
                        }
                    }
...
```"#;
    ...
    fn test_hard_split_multibyte() {
        ...
        let text = "帽".repeat(100);
        let splits = hard_split_by_chars(&text, 10);
        for split in &splits {
            assert!(split.len() <= 10 || split.len() <= "帽".len()); // May slightly exceed due to char boundary
        }
    }
