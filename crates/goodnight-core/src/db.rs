//! Database management for goodnight-core
...
/// A read-only connection handle 鈥?either borrowed from the pool or a temporary connection.
pub enum ReadConn<'a> {
...
impl Database {
    ...
    fn open_with_pool_size(
        ...
        // Always run migrations 鈥?idempotent, handles both new and existing DBs
    ...
    pub fn read_conn(&self) -> Result<ReadConn<'_>, AtomicCoreError> {
        ...
        // All pool slots busy 鈥?create a temporary connection
    ...
    pub fn run_migrations(conn: &Connection) -> Result<(), AtomicCoreError> {
        ...
        // --- V0 鈫?V1: Baseline schema (tables + indexes) ---
        if version < 1 {
            conn.execute_batch(
                r#"
        ...
        // --- V1 鈫?V2: Denormalize atom_count onto tags ---
        if version < 2 {
        ...
        // --- V2 鈫?V3: Add title and snippet columns to atoms ---
        if version < 3 {
            conn.execute_batch(
        ...
        // --- V3 鈫?V4: Add published_at column to atoms ---
        if version < 4 {
            conn.execute_batch(
        ...
        // --- V4 鈫?V5: Add source column (parsed from source_url) ---
        if version < 5 {
            conn.execute_batch(
        ...
        // --- V5 鈫?V6: Feed management tables ---
        if version < 6 {
            conn.execute_batch(
                r#"
        ...
        // --- V6 鈫?V7: Wiki article version history ---
        if version < 7 {
            conn.execute_batch(
                r#"
        ...
        // --- V7 鈫?V8: Store error reasons for failed embeddings/tagging ---
        if version < 8 {
            conn.execute_batch(
        ...
        // --- V8 鈫?V9: Wiki proposals (human-in-the-loop update review) ---
        if version < 9 {
            conn.execute_batch(
                r#"
        ...
        // --- V9 鈫?V10: Track semantic edge computation status per atom ---
        if version < 10 {
            conn.execute_batch(
                r#"
        ...
        // --- V10 鈫?V11: Mark which top-level tags the auto-tagger may extend ---
        if version < 11 {
            let has_col: bool = conn
                .query_row(
        ...
        // --- V11 鈫?V12: Daily briefings + briefing citations ---
        if version < 12 {
            conn.execute_batch(
                r#"
        ...
        // --- V12 鈫?V13: Materialized markdown atom links ---
        if version < 13 {
            conn.execute_batch(
                r#"
        ...
        // --- V13 鈫?V14: Durable atom pipeline queue ---
        if version < 14 {
            conn.execute_batch(
                r#"
