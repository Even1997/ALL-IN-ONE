...
/// discarded 鈥?we do not fuzzy-match.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[cfg_attr(feature = "openapi", derive(utoipa::ToSchema))]
#[serde(tag = "op")]
pub enum WikiSectionOp {
    /// No change 鈥?the new sources don't warrant updating the article.
    NoChange,
...
impl WikiSectionOpWire {
    ...
    pub fn into_op(self) -> Result<WikiSectionOp, String> {
        match self.op.as_str() {
            ...
            other => Err(format!(
                "Unknown op '{}' 鈥?expected NoChange, AppendToSection, ReplaceSection, or InsertSection",
                other
            )),
        }
    }
...
/// doesn't exist in the article 鈥?the caller should log both the missing
    ...
    for op in ops {
        match op {
            WikiSectionOp::NoChange => {
                // Tolerate 鈥?callers should short-circuit on this, but if a
...
";
    ...
    fn subsection_does_not_split_parent() {
        // Details has a ### Subsection 鈥?parsing must keep it inside Details.
