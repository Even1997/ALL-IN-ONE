...
pub enum WikiStrategy {
    /// Default 鈥?centroid similarity chunk selection + single-shot LLM call
    Centroid,
...
///
/// The applier has already been run 鈥?`merged_content` is the final article
...
///    shared across strategies 鈥?the LLM emits a list of section operations, the
...
///   for "not applicable" 鈥?same convention as `extraction.rs` which is
...
async fn generate_section_ops_proposal(
    ...
    let headings_block = if heading_list.is_empty() {
        "(no ## headings 鈥?the article has no sections yet; use InsertSection with after_heading=\"\" to add one at the end)".to_string()
    } else {
        heading_list
            .iter()
            .map(|h| format!("- {}", h))
    ...
    let user_content = format!(
        "CURRENT ARTICLE:\n{}\n\n\
         CURRENT SECTION HEADINGS (use these values verbatim in your operations 鈥?do not paraphrase):\n{}\n\n\
         EXISTING SOURCES (already cited in the article 鈥?reuse these indices verbatim if you reference them):\n{}\n\
    ...
    // required field) aborts the whole proposal 鈥?same posture as a
    ...
    // Apply ops to the existing content. Hallucinated heading 鈫?error propagates.
    ...
    // mapping (`[N]` 鈫?`chunks[N-1]`), which only works when the article's
    ...
    // build an explicit `index 鈫?source` map instead: existing citations map
    ...
    for cap in marker_re.captures_iter(&merged_content) {
        ...
        if let Some(existing_citation) = existing_by_index.get(&index) {
            // Preserve the original citation unchanged 鈥?same atom, same
            ...
            tracing::warn!(
                citation_index = index,
                "[wiki] Merged article references citation with no known source 鈥?dropping from citation list"
            );
        }
    }
...
/// `section_ops.rs` only treat `##` as a section boundary 鈥?`###` and deeper
...
- "ReplaceSection": rewrite a section's body (use sparingly 鈥?only when existing content is directly contradicted or superseded). Set `heading` to the exact existing section heading. Set `content` to the new body. Leave `after_heading` as an empty string.
...
- Keep tone consistent with the existing article.
- When mentioning topics that have their own wiki articles, use [[Topic Name]] notation 鈥?only for topics listed in EXISTING WIKI ARTICLES.
...
pub fn load_wiki_links(conn: &Connection, tag_id: &str) -> Result<Vec<WikiLink>, String> {
    ...
    // If no article exists, the subquery returns NULL and the WHERE matches nothing 鈥?
...
pub fn get_related_tags(
    ...
    // === Signal 1: Shared atoms (co-occurrence) 鈥?cheap self-join (~1ms) ===
    {
        let mut stmt = conn
            .prepare(
...
fn find_tags_mentioned_in_article(
    ...
    // Get article content 鈥?early return if no article (blank page)
    ...
    // We filter by name match in Rust, so most rows are discarded 鈥?no point counting atoms for all of them.
...
mod tests {
    ...
    // silent cross-provider divergence (wiki has been bitten by this before 鈥?
