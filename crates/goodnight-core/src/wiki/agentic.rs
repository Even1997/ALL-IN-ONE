...
async fn run_research(
    ...
    for iteration in 0..MAX_RESEARCH_ITERATIONS {
        ...
        let tool_calls = match response.tool_calls {
            ...
            _ => {
                // No tool calls 鈥?agent is done (or just sent text)
...
- Review each result critically 鈥?select only chunks with substantive, unique information
...
- You do NOT write the article 鈥?you only curate the sources"#
    )
}
...
fn research_user_prompt_update(tag_name: &str, existing_article: &str) -> String {
    format!(
        "Find and curate source material for updating the wiki article about \"{tag_name}\". \
         The existing article is shown below 鈥?focus on finding NEW information, \
...
/// is already present in the existing article's citations is dropped 鈥?the
...
pub(crate) async fn research_for_update(
    ...
    // Match on (atom_id, chunk_index) 鈥?the same key used when extracting
