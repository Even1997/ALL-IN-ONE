...
mod tests {
    ...
    fn test_truncate_preview_utf8_safe() {
        // ASCII shorter than limit 鈥?returned as-is
        assert_eq!(truncate_preview("hello", 100), "hello");

        // ASCII longer than limit 鈥?truncated with ellipsis
        let long_ascii = "a".repeat(150);
        let truncated = truncate_preview(&long_ascii, 100);
        assert_eq!(truncated.len(), 103); // 100 'a' + "..."
        assert!(truncated.ends_with("..."));

        // Cyrillic that would panic with byte slicing at 100 鈥?must not panic
        // and must return a valid UTF-8 string.
        let cyrillic = "协褌芯 锌褉懈屑械褉 写谢懈薪薪芯谐芯 褌械泻褋褌邪 薪邪 褉褍褋褋泻芯屑 褟蟹褘泻械, 泻芯褌芯褉褘泄 褋芯写械褉卸懈褌 褌芯谢褜泻芯 芯斜褘褔薪褘械 泻懈褉懈谢谢懈褔械褋泻懈械 褋懈屑胁芯谢褘 懈 薪械褋泻芯谢褜泻芯 锌褉械写谢芯卸械薪懈泄 锌芯写褉褟写. 袨薪 薪褍卸械薪 褌芯谢褜泻芯 写谢褟 胁芯褋锌褉芯懈蟹胁械写械薪懈褟 芯褕懈斜泻懈.";
        let preview = truncate_preview(cyrillic, 100);
        assert!(preview.ends_with("..."));
        assert_eq!(preview.chars().count(), 103); // 100 chars + "..."

        // Emoji (4-byte UTF-8) 鈥?must not panic
        let emoji = "馃榾".repeat(150);
    ...
    fn test_get_conversations_with_cyrillic_message() {
        ...
        save_message(&conn, &conv.conversation.id, "user", "袩褉懈胁械褌").unwrap();
        save_message(
            &conn,
            &conv.conversation.id,
            "assistant",
            "协褌芯 锌褉懈屑械褉 写谢懈薪薪芯谐芯 褌械泻褋褌邪 薪邪 褉褍褋褋泻芯屑 褟蟹褘泻械, 泻芯褌芯褉褘泄 褋芯写械褉卸懈褌 褌芯谢褜泻芯 芯斜褘褔薪褘械 泻懈褉懈谢谢懈褔械褋泻懈械 褋懈屑胁芯谢褘 懈 薪械褋泻芯谢褜泻芯 锌褉械写谢芯卸械薪懈泄 锌芯写褉褟写. 袨薪 薪褍卸械薪 褌芯谢褜泻芯 写谢褟 胁芯褋锌褉芯懈蟹胁械写械薪懈褟 芯褕懈斜泻懈 胁 芯斜褉邪斜芯褌泻械 preview 褋褌褉芯泻懈. 袝褋谢懈 褋懈褋褌械屑邪 锌褘褌邪械褌褋褟 芯斜褉械蟹邪褌褜 褌邪泻芯泄 褌械泻褋褌 锌芯 斜邪泄褌邪屑, 邪 薪械 锌芯 泻芯褉褉械泻褌薪芯泄 UTF-8 谐褉邪薪懈褑械 褋懈屑胁芯谢邪, 褋械褉胁械褉 锌邪写邪械褌 锌褉懈 蟹邪谐褉褍蟹泻械 褋锌懈褋泻邪 褉邪蟹谐芯胁芯褉芯胁.",
        )
        .unwrap();
        ...
        assert!(preview.starts_with("协褌芯 锌褉懈屑械褉"));
    }
