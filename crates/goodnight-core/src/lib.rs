...
//! goodnight-core: Knowledge base library for Atomic
//!
...
//!
//! ```rust,ignore
//! use goodnight_core::{AtomicCore, CreateAtomRequest, EmbeddingEvent};
//!
...
/// - `invalidate()` 鈥?eager clear for direct user mutations. Next read pays
///   full compute cost.
/// - `invalidate_debounced()` 鈥?keeps the stale payload visible and spawns
...
impl CanvasCache {
    ...
    /// to readers in the meantime. Only the latest scheduled rebuild wins 鈥?
...
pub struct AtomicCore {
    ...
    /// created lazily and persist for the lifetime of the process 鈥?the
    ...
    /// loop and the `POST /api/briefings/run` route contend for this lock 鈥?
...
impl AtomicCore {
    ...
    /// Run storage optimization 鈥?call on graceful shutdown.
    ...
    fn seed_and_backfill(
        ...
        // Only for empty databases (no atoms yet) 鈥?e.g. newly created databases
        ...
        F: Fn(EmbeddingEvent) + Send + Sync + Clone + 'static,
    {
        ...
        // Collect atom IDs for background embedding (don't clone content 鈥?read from DB later)
    ...
    /// Configure auto-tag targets in one shot 鈥?used by the onboarding wizard
    ...
    /// content, they're unflagged instead so their data isn't lost 鈥?re-runs from
    ...
    pub async fn generate_wiki(
        ...
        // and is now meaningless 鈥?drop it. Log-and-continue on error; a stale
    ...
    pub async fn accept_wiki_proposal(
            ...
            || existing.article.id != proposal.base_article_id
        {
            tracing::warn!(
                ...
                "[wiki] Stale proposal 鈥?live article was updated out-of-band"
            );
            return Err(AtomicCoreError::Wiki(
                "Proposal is stale 鈥?live article was updated since the proposal was computed. Regenerate the proposal to review it.".to_string(),
            ));
        }
    ...
    /// Delete a wiki article (and any pending proposal for it 鈥?once the
    ...
    ///
    /// Returns `Ok(None)` when there are no new atoms in the window 鈥?no
    ...
    /// error bubbles up and `last_run` is left unchanged 鈥?the scheduler
    ...
    /// task to re-embed them (with tagging skipped 鈥?existing tags are preserved).
        ...
        F: Fn(EmbeddingEvent) + Send + Sync + Clone + 'static,
    {
        ...
        // The claim flips every atom 'complete' 鈫?'processing' unconditionally,
    ...
    /// Pure read operation 鈥?does not persist positions to the database.
    ...
    pub async fn compute_and_get_canvas_data(
        ...
        // guard 鈥?if another waiter already populated the cache while we
        // blocked, use theirs.
    ...
    /// interaction 鈥?callers decide whether to read/write the cache. Takes
    ...
    async fn compute_canvas_data_impl(
        ...
        // 鈥?no full content, no embedding blobs, single query with LEFT JOIN
    ...
    /// Captures a `StorageBackend` clone 鈥?no reference cycle.
    ...
    /// pipeline is spawned 鈥?actual edge computation runs in the background
        ...
        F: Fn(EmbeddingEvent) + Send + Sync + Clone + 'static,
    {
        ...
        if embedding_space_keys.contains(&key) && value_changed {
            ...
            if old_dim != new_dim {
                tracing::info!(
                    ...
                    new_dim,
                    key,
                    "Embedding dimension change detected 鈥?recreating vector index and re-embedding all atoms"
                );
        ...
        if dimension_changed {
            ...
            // Now spawn re-embedding 鈥?atoms are in 'pending' status after the reset.
        ...
        if retry_keys.contains(&key) && !embedding_space_changed && value_changed {
            ...
            if retried_failed > 0 {
                tracing::info!(
                    retried_failed,
                    key,
                    "Provider config updated 鈥?retrying previously failed atoms"
                );
    ...
    /// Each URL is processed independently 鈥?individual failures don't affect others.
    ...
    where
        ...
        G: Fn(EmbeddingEvent) + Send + Sync + Clone + 'static,
    {
        // Fetch feed data (XML/JSON) 鈥?use shared HTTP client with proper User-Agent
        ...
        G: Fn(EmbeddingEvent) + Send + Sync + Clone + 'static,
    {
        ...
        // Fetch feed XML 鈥?use shared HTTP client with proper User-Agent
        ...
        for item in &parsed.items {
            // Claim the GUID atomically 鈥?if another poll already claimed it, skip.
...
mod tests {
    ...
    async fn test_global_search_returns_snippet_and_match_offsets() {
        ...
        // top level 鈥?the FTS excerpt must live under a distinct key so JSON
    ...
    async fn test_global_search_caps_match_offsets_but_reports_total() {
        ...
        // 25 occurrences of the token `zeta` 鈥?well past the cap of 10.
    ...
    async fn test_global_search_returns_wiki_snippet_and_offsets() {
        ...
        let offsets = hit
            ...
            .as_ref()
            .expect("match_offsets should be populated for keyword hits");
        // FTS5 phrase-quotes the query, so `zebra` matches whole tokens only 鈥?
