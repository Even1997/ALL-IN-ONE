...
pub enum AtomInput {
    ...
    /// Only atom IDs 鈥?content will be loaded per-group from storage
    IdsOnly(Vec<String>),
}
...
async fn process_tagging_only_inner(
    ...
    // Read raw content directly from atoms table 鈥?no dependency on embedding
    let content = storage
    ...
    // No auto-tag targets configured 鈥?skip tagging entirely.
    // The user has either unflagged all defaults during onboarding or hasn't created any.
    if tag_tree_json == "(no existing tags)" {
        tracing::info!(atom_id = %atom_id, "No tags flagged as auto-tag targets; skipping tagging (check Settings 鈫?Tagging)");
        storage
            .set_tagging_status_sync(atom_id, "skipped", None)
            .await
            .map_err(|e| e.to_string())?;
    ...
    for group_idx in 0..num_groups {
        ...
        for (chunk_group_idx, chunk_group) in chunk_groups.into_iter().enumerate() {
                ...
                .await
            {
                ...
                Err(e) => {
                    // Entire batch transaction failed 鈥?fall back to per-atom
...
/// Formula: cosine_similarity = 1 - (L2_distance虏 / 2)
/// This derives from: L2虏 = 2(1 - cos(胃)) for unit vectors
...
pub fn compute_tag_embeddings_batch(
    ...
    // descendant_tag_id 鈫?set of target tag_ids whose centroid it contributes to.
...
pub async fn process_pending_edges(
    ...
    crate::executor::spawn(async move {
        ...
        loop {
            ...
            // quick succession 鈥?successive calls collapse into one rebuild.
