...
/// Precomputed tag tree data 鈥?loaded once per request, reused across functions.
struct TagTree {
    /// (id, name, parent_id) for every tag
    all_tags: Vec<(String, String, Option<String>)>,
    /// tag_id 鈫?number of atoms directly tagged (not counting descendants)
    direct_counts: HashMap<String, i32>,
    /// tag_id 鈫?total atom count including all descendant tags (computed in one pass)
    transitive_counts: HashMap<String, i32>,
    /// tag_id 鈫?list of direct child tag IDs
    children_map: HashMap<String, Vec<String>>,
    /// tag_id 鈫?tag name (for fast lookup)
...
fn build_root_level(conn: &Connection) -> Result<CanvasLevel, AtomicCoreError> {
    ...
    if !unclustered_ids.is_empty() {
        if unclustered_ids.len() <= MAX_ATOMS_PER_LEVEL {
            ...
            // Too many 鈥?wrap in a single "Unclustered" bubble
...
fn build_tag_level(conn: &Connection, tag_id: &str) -> Result<CanvasLevel, AtomicCoreError> {
    ...
    if !child_ids.is_empty() {
        // This tag has child tags 鈥?show them as nodes
        ...
        if tag_nodes.len() <= MAX_TAGS_PER_LEVEL {
            ...
            if rest.len() <= MAX_TAGS_FOR_CLUSTERING {
                ...
                // Too many for semantic clustering 鈥?group by count ranking (O(n), no SQL)
...
fn build_hint_level(
    ...
    if found_tags.len() == hint_ids.len() {
        // All are tags 鈥?build tag nodes with counts
        let tree = TagTree::load(conn)?;
...
/// O(n) with zero SQL 鈥?just slices the sorted array.
...
/// Batch lookup tag names for each atom (returns atom_id 鈫?vec of tag names)
...
/// For large datasets (root level with 34K atoms), skip 鈥?not useful.
...
mod tests {
    ...
    fn test_root_level_shows_unclustered_atoms() {
        ...
        // Single atom with no edges 鈥?should appear as individual atom node
