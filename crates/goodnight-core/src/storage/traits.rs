//! Storage trait definitions for goodnight-core.
//!
...
pub trait AtomStore: Send + Sync {
    ...
    /// Does NOT trigger embedding 鈥?that's handled by AtomicCore.
...
pub trait SearchStore: Send + Sync {
    ...
    /// `created_after` is an optional ISO 8601 cutoff 鈥?only atoms created at or after
...
pub trait WikiStore: Send + Sync {
    ...
    /// Save a wiki proposal (upsert 鈥?supersedes any existing proposal for the tag).
