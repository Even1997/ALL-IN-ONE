...
impl SqliteStorage {
    ...
    pub(crate) fn keyword_search_sync(
        ...
        // Query atom-level FTS. Each row is already one atom 鈥?no chunk dedupe
...
/// consumer 鈥?palette, future API users, tests 鈥?the same truncated view.
        ...
        let mut stmt = conn
            .prepare(
                "SELECT f.id,
                        ...
                        snippet(atoms_fts, 1, '\u{E000}', '\u{E001}', '鈥?, 20) AS snippet,
        ...
        let mut stmt = conn
            .prepare(
                "SELECT f.id, f.atom_id, f.content, f.chunk_index,
                        ...
                        snippet(atom_chunks_fts, 3, '\u{E000}', '\u{E001}', '鈥?, 20) AS snippet
    ...
    let mut stmt = conn
        .prepare(
            "SELECT id, tag_id, tag_name, content,
                    ...
                    snippet(wiki_articles_fts, 3, '\u{E000}', '\u{E001}', '鈥?, 20) AS snippet,
