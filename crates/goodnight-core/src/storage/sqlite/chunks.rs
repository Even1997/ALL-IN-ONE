...
impl SqliteStorage {
    ...
    pub(crate) fn save_chunks_and_embeddings_batch_sync(
        ...
        for (atom_id, chunks) in atoms {
            ...
            match Self::save_chunks_for_atom(&sp, atom_id, chunks) {
                ...
                Err(e) => {
                    // sp is dropped without commit 鈫?implicit rollback to savepoint
    ...
    pub(crate) fn rebuild_semantic_edges_sync(&self) -> StorageResult<i32> {
        ...
        // handle 鈥?storage has no knowledge of the cache.
    ...
    /// Lightweight 鈥?no full SemanticEdge struct, no chunk indexes, no timestamps.
    ...
    /// runs a transaction, and releases 鈥?keeping mutex hold times short so
    ...
    pub(crate) fn compute_semantic_edges_batch_sync(
        ...
        for chunk in atom_ids.chunks(EDGE_SUB_BATCH) {
            ...
            tx.commit()
                .map_err(|e| AtomicCoreError::DatabaseOperation(e.to_string()))?;
            // conn lock dropped here 鈥?other writers can proceed between sub-batches
        }
        Ok(total_edges)
    }
