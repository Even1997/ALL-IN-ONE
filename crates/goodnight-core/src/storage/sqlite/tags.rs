...
impl SqliteStorage {
    ...
    pub(crate) fn get_tag_children_impl(
        ...
        // so no JOIN or GROUP BY needed 鈥?just read the column directly.
        let mut stmt = conn.prepare(
    ...
    pub(crate) fn configure_autotag_targets_impl(
        ...
        // Step 2: handle unrequested defaults 鈥?delete if empty, otherwise unflag.
        for t in &top_level {
        ...
        // Step 3: custom additions 鈥?create or flag in place. Re-query each
