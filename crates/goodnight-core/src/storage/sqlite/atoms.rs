...
impl SqliteStorage {
            ...
            if let Err(e) = (|| -> Result<(), AtomicCoreError> {
                ...
                if reset_embedding_status {
                    ...
                    if content_changed {
                        ...
                        // Content unchanged 鈥?FTS stays in sync without a resync.
                        conn.execute(
    ...
    pub(crate) fn list_atoms_impl(
        ...
        // Tag filter 鈥?recursive CTE to include full descendant subtree
    ...
    /// No full content, no embedding blobs 鈥?just what the canvas needs.
    ///
    /// Uses a single LEFT JOIN + GROUP BY instead of correlated subqueries so
    /// the query is O(atoms + tag_links) rather than O(atoms 脳 2) index lookups.
    /// `primary_tag` is MIN(name) 鈥?alphabetically stable, unlike the previous
