...
fn get_canvas_tools() -> Vec<ToolDefinition> {
    vec![
        ToolDefinition::new(
            "zoom_to_cluster",
            "Zoom the canvas camera to center on a single cluster of related atoms. The canvas can only show one view at a time 鈥?each call replaces the previous view. Use this when the user asks about a topic area visible on their knowledge graph.",
            json!({
                "type": "object",
                "properties": {
                    "cluster_label": {
...
The canvas shows a single view at a time 鈥?each navigation call replaces the previous one. If the user asks about multiple clusters or atoms, pick the most relevant one or navigate sequentially, not simultaneously.
...
async fn execute_get_atom(
    ...
    // Empty atom 鈥?return empty before the range math, otherwise a nonzero
