...
impl DatabaseManager {
    ...
    async fn resolve_database_id(&self, id_or_name: &str) -> Result<String, AtomicCoreError> {
        ...
        // Return the original value 鈥?let downstream handle not-found
        Ok(id_or_name.to_string())
    }

    /// Get a core for a specific database, loading it lazily if needed.
    /// Accepts either a database ID or name 鈥?if `id` doesn't match a known
    ...
    ///
    /// Returns `None` when the manager is running against Postgres 鈥?in that mode,
    ...
    pub async fn recreate_other_vector_indexes(
        ...
        #[cfg(feature = "postgres")]
        if self.is_postgres() {
            // In Postgres mode all databases share the same atom_chunks table 鈥?
