...
//!
//! - No `oneOf` / `anyOf` 鈥?some providers (notably OpenRouter-routed local
...
//!   advertise `structured_outputs`, but we don't gate here 鈥?the fallback
...
impl<'a, T> StructuredCall<'a, T> {
    ...
    /// `structured_output` field is always set by [`call_structured`] 鈥?
    ...
    /// `response_format`, just without the constrained-decoding guarantee 鈥?
...
/// so they can log / surface it sensibly 鈥?`ParseFailed` in particular carries
...
/// 4. Tolerantly parse the response (raw 鈫?strip code fences 鈫?locate first
///    `{鈥` substring).
            ...
            Ok(response) => {
                ...
                match parse_tolerant::<T>(&response.content) {
                    ...
                    Err(parse_err) => {
                        ...
                        // Parse errors don't retry on the same call shape 鈥?break out
...
fn collect_lint_errors(node: &Value, path: &str) -> Vec<String> {
    ...
    if obj.contains_key("oneOf") {
        errors.push(format!(
            "{}: oneOf is not portable 鈥?use a flat schema with a discriminator field",
            here
        ));
    }
    if obj.contains_key("anyOf") {
        errors.push(format!(
            "{}: anyOf is not portable 鈥?use a flat schema with a discriminator field",
            here
        ));
    }
    ...
    if let Some(type_val) = obj.get("type") {
        if let Some(type_arr) = type_val.as_array() {
            ...
            if has_null {
                errors.push(format!(
                    "{}: nullable union types are not portable 鈥?use an empty-string sentinel instead",
                    here
                ));
            }
        }
    ...
    for (key, value) in obj {
        // Skip re-linting the required/type arrays themselves 鈥?they're metadata,
        // not nested schemas.
...
fn strip_code_fences(content: &str) -> String {
    ...
    if let Some(rest) = trimmed.strip_prefix("```") {
        ...
        // No trailing fence 鈥?return what we have after the opening fence.
...
mod tests {
    ...
    // primary 鈫?parse 鈫?(retry | fallback) 鈫?parse 鈫?Ok | Err. Each test
    ...
    async fn pipeline_fenced_json_parses_without_fallback() {
        ...
        // Tolerant parse handled the fence 鈥?no fallback call needed.
    ...
    async fn pipeline_retry_exhausted_returns_provider_error() {
        let provider = Arc::new(MockLlmProvider::new());
        // max_retries = 2 鈫?1 primary + 2 retries = 3 failing calls before
    ...
    async fn pipeline_fallback_call_never_sends_schema() {
        ...
        // That call should NOT carry `structured_output` 鈥?the nudge prompt
    ...
    async fn pipeline_schema_lint_fails_before_any_call() {
        // Schema with oneOf 鈥?lint rejects, we never reach the provider.
        let bad_schema = json!({
            "type": "object",
            "properties": {
    ...
    // snapshot catches semantic drift that the lint tests can't see 鈥?e.g.
    ...
    //
    // Regenerate with:
    //   UPDATE_SNAPSHOTS=1 cargo test -p goodnight-core schema_snapshot
    //
    ...
    // `preserve_order` feature (utoipa does, so an goodnight-server workspace
    // build unifies it on). That means we can't rely on serialization order
    // 鈥?canonicalize by recursively sorting object keys before comparing.
        ...
        let expected = std::fs::read_to_string(&path).unwrap_or_else(|e| {
            panic!(
                "Snapshot file missing at {}: {}\n\n\
                 Run: UPDATE_SNAPSHOTS=1 cargo test -p goodnight-core schema_snapshot",
                path.display(),
                e
            )
        });
