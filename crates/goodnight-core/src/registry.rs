...
impl Registry {
    ...
    pub fn open_or_create(data_dir: impl AsRef<Path>) -> Result<Self, AtomicCoreError> {
        ...
        if needs_legacy_migration {
            ...
            // Create default database entry 鈥?the actual .db file is created
    ...
    /// Does NOT delete the .db file 鈥?the caller (DatabaseManager) handles that.
...
mod tests {
    ...
    fn test_legacy_migration() {
        ...
        // Open registry 鈥?should trigger legacy migration
        let registry = Registry::open_or_create(dir.path()).unwrap();
