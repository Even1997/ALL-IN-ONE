//! Content ingestion pipeline 鈥?URL fetching, article extraction, and RSS feed management.
//!
//! Two-layer architecture:
//! 1. `resolve_url()` 鈥?fetch + extract, no DB interaction
//! 2. `AtomicCore::ingest_url()` 鈥?dedup, create atom, trigger embedding
...
pub enum IngestionEvent {
    ...
    /// Page wasn't article-shaped 鈥?no atom created.
    Skipped {
        url: String,
        request_id: String,
        reason: String,
...
/// Resolved content from a URL 鈥?the output of fetch + extract, before DB writes.
