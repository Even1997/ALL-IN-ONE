...
//! *global*, not per-DB). Anything that needs per-DB state 鈥?scheduler
//! last-run timestamps, per-DB feature flags 鈥?must bypass that routing.
//!
...
//!   3. `set_setting` is *intentionally global* when a registry is attached 鈥?
...
use goodnight_core::{CreateAtomRequest, DatabaseManager};
...
async fn isolation_postgres() {
    ...
    // Shared Postgres deployment 鈥?start clean so leftover rows from earlier
...
async fn run_isolation(manager: &DatabaseManager) {
    ...
    // quietly swaps this to per-DB routing, this assertion fires 鈥?giving
    ...
    assert_eq!(
        ...
        "set_setting/get_settings share state across DBs (registry when SQLite, \
         per-db_id rows in Postgres). Per-DB state MUST use a different mechanism 鈥?\
         see crates/goodnight-core/src/scheduler/state.rs for the canonical pattern."
    );
}
