...
//!
//! One atom creation 鈫?chunk 鈫?embed (via mock HTTP) 鈫?auto-tag (via mock HTTP)
//! 鈫?deferred graph maintenance. Verifies every persisted artifact at the end.
//!
...
use goodnight_core::{AtomicCore, CreateAtomRequest, EmbeddingEvent, UpdateAtomRequest};
use support::{
...
async fn run_full_pipeline(backend: Backend) {
    ...
    // chunk each 鈥?simplifies assertions and keeps the request count
    // predictable.
    ...
    // --- Tagging phase: the mock returned Physics鈫扵opics, and the pipeline
    ...
    // threshold. With B created second, the edge is stored source=B鈫扐.
    let edges = core
        .get_semantic_edges(EDGE_SIMILARITY_THRESHOLD)
        .await
        .unwrap();
...
async fn run_update_lifecycle(backend: Backend) {
    ...
    // Vocabulary A 鈥?atoms a and b land near each other.
...
/// every semantic edge it participates in. Tags survive 鈥?they're shared
...
async fn run_delete_cascade(backend: Backend) {
    ...
    // top-level rows with children nested inside 鈥?simpler to grab the
    ...
    // Physics tag is shared state 鈥?still present, now only linked to b.
    let remaining = core
...
fn involves(edge: &goodnight_core::SemanticEdge, a: &str, b: &str) -> bool {
