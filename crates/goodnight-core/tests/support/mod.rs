...
//! the usual settings 鈥?the real reqwest client, real request serialization,
...
impl MockAiServer {
    pub async fn start() -> Self {
        ...
        // the request body so the same mock can serve any structured call 鈥?
    ...
    /// Base URL the `OpenAICompatProvider` should hit. No `/v1` suffix 鈥?
...
/// the same positions 鈫?high cosine similarity 鈫?edge crosses the 0.5
...
fn embed_text(text: &str) -> Vec<f32> {
    ...
    if norm > 0.0 {
        ...
        // Empty/punctuation-only input 鈥?put a constant at position 0 so
...
/// 鈥?the temp dir needs to live until after the core is dropped (SQLite has
...
pub async fn setup_core(backend: Backend, mock_url: &str) -> Option<CoreHandle> {
    let (core, tempdir) = match backend {
        ...
        Backend::Postgres => {
            ...
            // Fresh schema per test run 鈥?truncate leaves the schema intact
...
pub async fn truncate_postgres_for_test(url: &str) {
    ...
    // Same list as storage_tests.rs 鈥?keeps both files in sync.
    let _ = sqlx::raw_sql(
...
pub type EventRx = UnboundedReceiver<goodnight_core::EmbeddingEvent>;
...
pub fn event_collector() -> (
    impl Fn(goodnight_core::EmbeddingEvent) + Send + Sync + Clone + 'static,
    EventRx,
) {
...
pub async fn await_pipeline(rx: &mut EventRx, atom_id: &str) -> Vec<goodnight_core::EmbeddingEvent> {
    use goodnight_core::EmbeddingEvent;

    let mut captured = Vec::new();
    let mut embedding_done = false;
    let mut tagging_done = false;

    // A generous budget 鈥?the mock responds instantly, but CI runners can
