...
//!
//! Usage:
//!   cargo test -p goodnight-core --test storage_tests                         # SQLite only
//!   cargo test -p goodnight-core --test storage_tests --features postgres     # Both
//!
//! Note: Postgres tests must run serially (they share one DB):
//!   cargo test -p goodnight-core --test storage_tests --features postgres -- postgres_tests --test-threads=1

use goodnight_core::db::Database;
use goodnight_core::models::*;
use goodnight_core::storage::traits::*;
use goodnight_core::storage::SqliteStorage;
use goodnight_core::{CreateAtomRequest, ListAtomsParams, UpdateAtomRequest};
use std::sync::Arc;
...
async fn postgres_storage() -> Option<goodnight_core::storage::PostgresStorage> {
    let url = match std::env::var("ATOMIC_TEST_DATABASE_URL") {
        Ok(url) => url,
        Err(_) => return None,
    };
    let storage = goodnight_core::storage::PostgresStorage::connect(&url, "test")
        .await
        .unwrap();
...
async fn test_create_and_get_tags(storage: &dyn TagStore) {
    ...
    // get_all_tags returns a tree 鈥?flatten to count
