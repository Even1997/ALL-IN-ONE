...
function Start-DevServerIfNeeded {
  ...
  $outLog = Join-Path $PWD ".tmp-builtin-ai-file-ops-vite.out.log"
  $errLog = Join-Path $PWD ".tmp-builtin-ai-file-ops-vite.err.log"
...
try {
  $tempStdout = [System.IO.Path]::Combine($env:TEMP, "goodnight-built-in-file-ops-$PID.stdout.log")
  $tempStderr = [System.IO.Path]::Combine($env:TEMP, "goodnight-built-in-file-ops-$PID.stderr.log")
  ...
  $nodeScript = (Join-Path $PSScriptRoot "test-builtin-ai-file-ops.cjs")
  ...
  if ($LASTEXITCODE -ne 0) {
    ...
    throw "Built-in AI file ops smoke test failed."
  }
} finally {
  Remove-Item -LiteralPath $tempStdout -Force -ErrorAction SilentlyContinue
  Remove-Item -LiteralPath $tempStderr -Force -ErrorAction SilentlyContinue
