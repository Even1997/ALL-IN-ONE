...
class AIService {
    ...
    toolExecutor = new ToolExecutor(DEFAULT_PROJECT_ROOT);
    setConfig(config) {
    ...
    async request(request, handler) {
        ...
        try {
            ...
            const content = await this.runAgentLoop([{ role: 'user', content: fullRequest.prompt }], systemPrompt, abortController.signal, handler);
            handler.onComplete({
                requestId,
                status: 'completed',
                content,
    ...
    async chat(prompt, handlers) {
        if (!this.config.apiKey) {
            return `AI 未配置。请先在设置中填写第三方 API Key、Base URL 和模型。\n\n推荐配置：\n- Provider: openai-compatible\n- Base URL: ${DEFAULT_BASE_URL}\n- Model: 由第三方平台提供`;
        }
        const content = await this.runAgentLoop([{ role: 'user', content: prompt }], this.buildChatSystemPrompt(), undefined, handlers?.onChunk
            ? {
                onStart: () => undefined,
...
拿到工具结果后，继续完成任务，不要停在工具调用本身。`;
    ...
    async simulateStream(requestId, prompt, handler) {
    ...
    async runAgentLoop(inputMessages, systemPrompt, signal, handler) {
    ...
    emitChunkText(text, handler, type = 'text') {
        if (!handler) {
            return;
        }
        const chunks = text.split(/(\s+)/).filter(Boolean);
        chunks.forEach((chunk) => handler.onChunk({
            type,
            content: chunk,
            timestamp: Date.now(),
        }));
    }
    async callProvider(messages, systemPrompt, signal) {
    ...
    async callOpenAICompatible(messages, systemPrompt, signal) {
    ...
    async callAnthropic(messages, systemPrompt, signal) {
