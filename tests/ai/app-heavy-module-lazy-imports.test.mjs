...
test('app lazily imports heavy AI service and design style-pack modules', async () => {
  const [appSource, persistenceSource] = await Promise.all([
    readFile(appPath, 'utf8'),
    readFile(persistencePath, 'utf8'),
  ]);

  assert.match(appSource, /const loadAIServiceModule = \(\) =>/);
  assert.match(appSource, /import\('\.\/modules\/ai\/core\/AIService'\)/);
  assert.doesNotMatch(appSource, /import\s+\{\s*aiService\s*\}\s+from ['"]\.\/modules\/ai\/core\/AIService['"]/);
