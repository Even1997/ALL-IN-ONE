...
test('removed knowledge-runtime built-in skills are absent from the built-in tree', async () => {
  for (const skillId of ['goodnight-m-flow', 'goodnight-llmwiki', 'goodnight-rag']) {
    await assert.rejects(access(path.join(builtinRoot, skillId)));
  }
});
