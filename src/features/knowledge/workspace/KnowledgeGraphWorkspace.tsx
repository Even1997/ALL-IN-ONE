  ...
  return (
    <section className="gn-graph-shell">
      <header className="gn-graph-header">
        <div>
          <span className="gn-note-eyebrow">{isGlobal ? 'Global Wiki Graph' : 'Focused Wiki Graph'}</span>
          <h2>{isGlobal ? 'Wiki 图谱' : '当前知识图谱'}</h2>
          <p>
            {isGlobal
              ? '这里展示整个项目的知识网络，不会强制锁定某个中心点。点击任意节点会直接打开对应知识笔记。'
      ...
      <div className="gn-graph-meta">
        <span>{graph?.nodes.length || 0} 个节点</span>
      ...
      <div className="gn-graph-stage">
        {graph ? (
          ...
          <div className="gn-graph-empty">
            <h3>{isGlobal ? '项目里还没有可展示的 Wiki 图谱' : '当前笔记暂时还没有关系图'}</h3>
            <p>
              {isGlobal
                ? '先整理知识库或创建一些知识笔记，系统就能在这里聚合出全局关系网络。'
            ...
            <div>
              ...
              <p>
                {selectedNote
                  ...
                  : isGlobal
                    ? '全局图谱会优先连接 Wiki 索引、AI 摘要和普通知识笔记之间的关联。'
