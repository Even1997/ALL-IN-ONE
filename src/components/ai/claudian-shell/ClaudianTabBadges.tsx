...
export const ClaudianTabBadges: React.FC = () => {
  ...
  return (
    <div className="claudian-tab-badges">
      {(visibleSessions.length > 0 ? visibleSessions : [{ id: 'placeholder', title: '会话 1' }]).map((session, index) => {
        ...
        return (
          <button
            key={session.id}
            type="button"
            className={`claudian-tab-badge ${isActive ? 'claudian-tab-badge-active' : 'claudian-tab-badge-idle'}`}
