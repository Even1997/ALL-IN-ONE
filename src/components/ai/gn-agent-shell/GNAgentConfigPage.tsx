  ...
  return (
    <section className="gn-agent-shell-page gn-agent-shell-config-page">
      <header className="gn-agent-shell-page-header gn-agent-shell-page-header-spread">
        ...
        <span className="gn-agent-shell-page-note">
          {isLoading
            ? '正在读取本地 .claude / .codex 配置…'
            : snapshot
              ? snapshot.homeDir
              : '当前环境无法读取本地配置'}
        </span>
      ...
      <div className="gn-agent-settings-tabs" role="tablist" aria-label="GN Agent settings">
        {[
          ...
          <button
            ...
            className={`gn-agent-settings-tab${activeTab === tab.id ? ' active' : ''}`}
      ...
      {activeTab === 'general' ? (
        <div className="gn-agent-shell-config-grid">
          ...
          <ConfigCard
            ...
            desc={
              snapshot
                ? '已从本地用户目录读取 GN Agent 对接所需的 provider 环境。'
                : '当前环境还没有读取到本地 provider 目录。'
            }
          ...
          {providerCards.map((provider) => (
            <ConfigCard
              ...
              desc={
                provider.enabled
                  ? `${provider.displayName} provider 已注册到 GN Agent 宿主。`
                  : `${provider.displayName} provider 尚未启用。`
              }
            />
          ))}
        </div>
          ...
          <ConfigCard
            ...
            desc={
              claudeConfigs.length > 0
                ? `已发现 ${claudeConfigs.length} 个可绑定配置。`
                : '当前应用内没有可绑定的 Anthropic 配置。'
            }
          >
          ...
          <ConfigCard
            ...
            desc={
              codexConfigs.length > 0
                ? `已发现 ${codexConfigs.length} 个可绑定配置。`
                : '当前应用内没有可绑定的 OpenAI Compatible 配置。'
            }
          >
            <div className="gn-agent-shell-config-list">
